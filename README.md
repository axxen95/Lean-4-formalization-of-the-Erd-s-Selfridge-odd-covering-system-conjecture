...
| File | Description |
|------|-------------|
| `ErdosSelfridge_v17_fixed.lean` | Lean 4 formalization (790 lines) |
| `erdos_selfridge_paper_v4.pdf` | Paper (6 pages) |
| `erdos_selfridge_paper_v4.tex` | LaTeX source |
| `aristotle_instruction_v17_fixed.txt` | Aristotle verification instructions |

## Architecture

### Axioms (2, BBMST published)

1. **`sieve_criterion`** (BBMST Thm 3.1 + 3.2): If sieve sum with actual block sizes < 1, with valid parameters (ca1 ≥ 1), covering is impossible.
2. **`sf_sieve_lt_one`** (BBMST Thm 1.1): For any odd primes, ∃ parameters (ca1 ≥ 1) with SF sieve sum < 1.
...
- **v17-fixed** (2026-05-07): Added `1 ≤ ca1` validity condition to `sieve_criterion` axiom. Prevents trivial instantiation with ca1=0. No sorry, 2 correct axioms.
